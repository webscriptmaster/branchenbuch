 ...
 * -------------------------------------------------------------
 */
function smarty_modifier_spanvorwahl($number)
{
	...
	if($number[0]==='0') {
		...
		if(in_array((integer)$test, $vorwahlen)) {
			$vorwahl = substr($number, 0, 5);
		} else {
			$test = substr($number, 1, 3);
			if(in_array((integer)$test, $vorwahlen)) {
				$vorwahl = substr($number, 0, 4);
			} else {
				$test = substr($number, 1, 2);
				if(in_array((integer)$test, $vorwahlen)) {
					$vorwahl = substr($number, 0, 3);
				} elseif($number[1]=='3') {
					$vorwahl = substr($number, 0, 6);
				} else {
					$vorwahl = substr($number, 0, 5);
				}
			}
		}
		return '<span class="vw'.strlen($vorwahl).'">'.$number.'</span>';
	} else {
		return $number;
	}
}
