...
function AddPage($orientation='',$revision)
{
	//Start a new page
	...
	switch($dest)
	{
		...
		case 'D':
			...
			*/
			...
			if(isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'],'MSIE')) header('Cache-control: private');
			header('Content-type: application/pdf');
			header('Content-Length: '.strlen($this->buffer));
			header('Content-Disposition: inline; filename='.$name);
			print $this->buffer;
